...
# On August 6, 2016 Todd Vaziri tweeted External link about Trump that "Every 
 # non-hyperbolic tweet is from iPhone (his staff). Every hyperbolic tweet is 
 # from Android (from him)." Data scientist David Robinson conducted an analysis
 # External link to determine if data supported this assertion. Here we go 
 # through David's analysis to learn some of the basics of text mining. 
# To learn more about text mining in R we recommend this book External link.
...
# We notice a big peak for the Android in early hours of the morning, between 
 # 6 and 8 AM. There seems to be a clear different in these patterns. We will 
...
#The main function needed to achieve this is unnest_tokens(). A token refers to
 ...
 # will be words, but they can also be single characters, ngrams, sentences, 
 # lines or a pattern defined by a regex. The functions will take a vector of
 # strings and extract the tokens so that each one gets a row in the new table.
...
# Note that the function tries to convert tokens into words and strips characters
 # important to twitter such as # and @. A token in twitter is not the same as
 # in regular English. For this reason, instead of using the default token, 
 # words, we define a regex that captures twitter character. The pattern appears
 # complex but all we are defining is a patter that starts with @, # or neither
...
stop_words
# If we filter out rows representing stop words with filter
...
# Some exploration of the resulting words (not show here) reveals a couple of 
 ...
 # numbers (years for example). We want to remove these and we can find them 
 # using the regex ^\d+$. Second, some of our tokens come from a quote and they 
 # start with '. We want to remove the ' when it's at the start of a word, so we
 # will use str_replace(). We add these two lines to the code above to generate
 # our final table:
...
# We already see somewhat of a pattern in the types of words that are being 
 ...
 # in specific words but rather in the tone. Vaziri's assertion is that the 
 # Android tweets are more hyperbolic. So how can we check this with data?
...
# Sentiment Analysis
# In sentiment analysis we assign a word to one or more "sentiment". Although 
 # this approach will miss context dependent sentiments, such as sarcasm, when
 # performed on large numbers of words, summaries can provide insights.
...
# Now we are ready to perform a quantitative analysis comparing Android and 
 ...
 # each tweet. However, this somewhat complex since each tweet will have several
 # sentiments attached to it, one for each word appearing in the lexicon. 
 # For illustrative purposes, we will perform a much simpler analysis: we will 
